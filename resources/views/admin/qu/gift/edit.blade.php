...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('qu/admin/gift/'.$gi->gi_id)}}" method="post">
            ...
            <table class="add_tab">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <select name="gi_type" onchange="option(this,'gi_type_content')" style="width:337px;font-size:14px;">
                                <option value="text" @if($gi->qu_type == 'text') selected @endif>發送序號</option>
                                <option value="img" @if($gi->qu_type == 'img') selected @endif>發送圖片</option>
                            </select>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th>名稱：</th>
                        <td>
                            <input type="text" size="50" name="gi_name" value="{{$gi['gi_name']}}">
                    ...
                    <tr>
                        <th>內容：</th>
                        <td>
                            <textarea id="gift-ckeditor" name="gi_content">{!!  $gi['gi_content'] !!}</textarea>
                    ...
                    <tr>
                        ...
                        <td id="gi_type_content">
                            <input type="number" size="50" onchange="subject(this,'qu_topic_list')" name="serial_number"  min="1" max="999" value="{{$gi['serial_number']}}">
                        </td>
                    </tr>

                    <tr id="qu_topic" @if($gi['serial_number'] == 0) style="display: none;" @endif>
                    {{--<tr id="qu_topic">--}}
                        <th>問卷題目：</th>
                        <td id="qu_topic_list">
                            @if($gi['gi_type'] == 'text')
                                @foreach($gi_content as $k=>$v)
                                    <div id='de_{{$k+1}}'>選項: <input  id='serial_number_list_{{$k}}' type='text' size='40' onchange='redirectpage(this)'  name='serial_number_list[]' value="{{$v->content}}">
                                    <span id='serial_number_list_amount_{{$k}}'>數量: <input type='number' min='1' max='9999' name='serial_number_list_amount[]' value='{{$v->amount}}'></span></div>
                                @endforeach
                            @endif
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td size="50">
                            <select name="gi_status" style="width:337px;font-size:14px;">
                                <option value="1" @if($gi['gi_status'] == 1) selected @endif>online</option>
                                <option value="0" @if($gi['gi_status'] == 0) selected @endif>offline</option>
                            </select>
                        </td>
                    </tr>
    ...
    <script>
        ...
        var now_number = "{{$gi['serial_number']}}";
