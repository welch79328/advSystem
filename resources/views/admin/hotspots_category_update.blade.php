...
<div id="content" class="col-lg-9 col-sm-9">
    ...
    <div>
        <ul class="breadcrumb">
            ...
            <li>
                <a href="javascript:;">修改熱點分類 (Update Hotspot Category)</a>
            </li>
        </ul>
    </div>
        ...
        <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-edit"></i>修改熱點分類(Update Hotspot Category)</h2>
                </div>
                <div class="box-content">
                    <form action="{{url('admin/update_hotspots_category/' . $info->swarmId)}}" method="post" enctype="multipart/form-data" role="form">
                        {{csrf_field()}}
                        <div class="form-group">
                            <label for="swarmName">熱點分類名稱 (Hotspot Category Name)</label>
                            <input name="swarmName" type="text" class="form-control" id="swarmName" placeholder="熱點分類名稱 (Hotspot Category Name)"  value="{{$info->swarmName}}"required>
                        </div>
                        <div class="form-group">
                            <label for="checked_hotspots">已勾選的熱點 (Checked Hotspots)</label>
                            <div id="checked_hotspots" class="form-control4" style="min-height: 100px;  height: auto; word-wrap: break-word">
                                @foreach($hotspots as $hotspot)
                                <span id="span{{$hotspot->rid}}" style="padding-right: 10px;">{{$hotspot->aliasName}}</span>
                                <input id="{{$hotspot->rid}}" type="hidden" name="rids[]" value="{{$hotspot->rid}}">
                                @endforeach
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div>
                            <button type="submit" class="btn btn-default">修改熱點分類 (Update Hotspot Category)</button>
